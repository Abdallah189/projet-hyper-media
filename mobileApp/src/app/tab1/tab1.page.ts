    ...
    for (let index = 0; index < this.reponse.length; index++) {
      ...
      if (test) {
        Swal.mixin({
          ...
          {
            input: 'textarea',
            inputPlaceholder: 'Ajouter des informations ou un autre type de fatigue si vous avez',
            inputAttributes: {
            ...
            this.rp.reponsePost(this.SendBodyFinal,this.token).subscribe((Response)=>{
              ...
              if(Response['message']=="Survey created"){
                Swal.fire({
                  ...
                  confirmButtonText:'<ion-icon src="../../assets/icon/thumbs-up-solid.svg" style=""></ion-icon> OK',
                  confirmButtonColor:'red',
                  text: 'Nous vous répondrons bientôt et vous souhaitons un prompt rétablissement!',
                })
